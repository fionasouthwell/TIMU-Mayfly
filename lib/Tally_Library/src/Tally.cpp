...
This library is used to interface to the Tally event counter module and allow for easy
reading and resetting of the device

"At each increase of knowledge, as well as on the contrivance of every new tool, human
labour becomes abridged." -Charles Babbage
...
Tally::Tally(uint8_t ClockPin_, uint8_t DataPin_, bool ClockLogic_)
{
  ...
  ClockLow = ClockLogic_;  // Copy to global variables, used to invert clock output to
...
uint8_t Tally::begin(uint8_t ClockPin_, uint8_t DataPin_, bool ClockLogic_)
{
  ...
  ClockLow = ClockLogic_;  // Copy to global variables, used to invert clock output to
                           // correct for hardware
  return begin();  // Call standard begin
}
...
unsigned int Tally::GetTicks()  // Read out number of ticks since last reset
{
  ...
  digitalWrite(ClockPin,
               !ClockLow);  // Pulse clock pin high for >5ms to load new data
  delay(5.5);
